  ...
  while ( i < argc ) {
    if ( *argv[i] == '-' ) {
...
#ifdef WITH_CUDA
      /* ---- CUDA strategy and checkpoint flags (checked before main switch) ---- */
      ...
      if ( !strcmp(argv[i],"-save-dir") && i + 1 < argc ) {
        mesh->info.cuda_save_dir = argv[++i];
        i++; continue;
      }
      if ( !strcmp(argv[i],"-save-all") ) {
        mesh->info.cuda_save_all = 1;
        i++; continue;
      }
      if ( !strcmp(argv[i],"-save-at") && i + 1 < argc ) {
        {
          const char *sn[] = {"post-load","post-scale","post-quality1","post-analys",
            "post-defsiz","post-gradsiz","post-quality2","post-adapt","post-unscale"};
          int si, found = 0;
          ++i;
          for (si = 0; si < 9; ++si) {
